...
describe("Strategy", function() {
    ...
    it("should verify ethereum address", function(done) {
        chai.passport
            .use(
                new Strategy(function(address, network, cb) {
    ...
    it("should verify Matic address", function(done) {
        chai.passport
            .use(
                new Strategy(function(address, network, cb) {
                    expect(address).to.equal(
                        "5K8DR5aAg8LY87xjxwvgqGt58hAPQ5MYdNXbmu7iKY9C"
                    );
                    expect(network).to.equal("Matic");
                    return cb(null, { id: "248289761001" });
                })
            )
            .request(function(req) {
                ...
                req.body = {
                    ...
                    payload: {
                        ...
                        statement: "Sign in with Matic to the app.",
                        uri: "http://localhost:3000",
                        version: "1",
                        chainId: 1,
            ...
            .error(done)
            .authenticate();
    }); // should verify Matic address

    it("should verify address and chain id", function(done) {
        chai.passport
            .use(
                new Strategy(function(address, chainId, cb) {
                    expect(address).to.equal(
                        "0xCC6F4DF4B758C4DE3203e8842E2d8CAc564D7758"
                    );
    ...
    it("should fail when URI is invalid", function(done) {
        chai.passport
            .use(
                new Strategy(function(address, cb) {
                    expect(address).to.equal(
                        "0xCC6F4DF4B758C4DE3203e8842E2d8CAc564D7758"
                    );
                    return cb(null, { id: "248289761001" });
        ...
        chai.passport
            ...
            .fail(function(challenge, status) {
                expect(challenge).to.deep.equal({
                    message: "Invalid signature.: 0x09967aCB4912a3efDb66039b8BC8ABb202a0f3E4 !== 0xCC6F4DF4B758C4DE3203e8842E2d8CAc564D7758",
                });
                expect(status).to.equal(403);
                done();
            })
